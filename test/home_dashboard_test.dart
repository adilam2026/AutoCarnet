...
void main() {
  ...
  group('multi-vehicle: every home section is scoped to the active vehicle only', () {
    testWidgets(
        ...
        'for the newly active vehicle only', (tester) async {
      final reminders = ReminderRepository(db);
      final repo = VehicleRepository(db, AuditRepository(db), reminders);
      final astraId =
    ...
    testWidgets(
        ...
        'reminder from the previous vehicle on screen', (tester) async {
      final reminders = ReminderRepository(db);
      final repo = VehicleRepository(db, AuditRepository(db), reminders);
      final astraId =
          await repo.createVehicle(brand: 'Opel', model: 'Astra', currentMileage: 270000);
      ...
      // Back on the Astra (first vehicle): only its own reminder shows.
      expect(find.text('Vidange Astra'), findsOneWidget);
      expect(find.text('Révision Q5'), findsNothing);
    });
    ...
    testWidgets(
        ...
        'vehicle in the garage has several', (tester) async {
      final reminders = ReminderRepository(db);
      final repo = VehicleRepository(db, AuditRepository(db), reminders);
      await repo.createVehicle(brand: 'Dacia', model: 'Duster', currentMileage: 50000);
      final busyId = await repo.createVehicle(brand: 'Audi', model: 'Q5', currentMileage: 86750);
      for (var i = 0; i < 5; i++) {
        await reminders.upsertForSource(
          vehicleId: busyId,
          sourceType: 'maintenance',
      ...
      expect(find.text('Tout est à jour'), findsOneWidget);
      expect(find.textContaining('est à jour.'), findsOneWidget);
      expect(find.textContaining('Opération 0'), findsNothing);
    });
  });
  ...
  group('"Dernières opérations" on the home dashboard', () {
    testWidgets(
      ...
      final repo = VehicleRepository(db, AuditRepository(db), reminders);
      final maintenance = MaintenanceRepository(db, timeline, reminders, repo);
      final vehicleId =
          await repo.createVehicle(brand: 'Audi', model: 'Q5', currentMileage: 90000);
    ...
    testWidgets('with no operations yet, shows a friendly empty message and no "Voir tout"',
        (tester) async {
      final repo = VehicleRepository(db, AuditRepository(db), ReminderRepository(db));
      await repo.createVehicle(brand: 'Audi', model: 'Q5', currentMileage: 90000);
      ...
      expect(find.textContaining('Aucune opération enregistrée'), findsOneWidget);
    });

    testWidgets(
        'switching to another vehicle shows that vehicle\'s own recent operations, not the '
        'previous one\'s', (tester) async {
      final reminders = ReminderRepository(db);
      final timeline = TimelineRepository(db);
      final repo = VehicleRepository(db, AuditRepository(db), reminders);
      final maintenance = MaintenanceRepository(db, timeline, reminders, repo);
