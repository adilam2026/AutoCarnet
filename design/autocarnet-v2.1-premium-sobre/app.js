(() => {
  ...
  const VEHICLES = {
    q5: {
      ...
      expense: '18 450', lastMaintWhen: 'Il y a 2 mois', lastMaintWhat: 'Vidange', kmYear: '13 200',
    },
    astra: {
      index: 1,
      name: 'Opel Astra',
      ...
      expense: '9 200', lastMaintWhen: 'Il y a 3 semaines', lastMaintWhat: 'Plaquettes', kmYear: '15 800',
    },
  };
  ...
  function showScreen(id) {
    ...
    if (bottomNav) {
      ...
      bottomNav.classList.toggle('is-hidden', id === 'login');
    }
    if (fab) fab.classList.toggle('is-visible', id === 'home');
    if (statusBar) statusBar.classList.toggle('is-on-brand', id === 'login');
    closeDrawer();
    closeSheet();
  ...
  function renderVehicleData(key) {
    ...
    if (todoHost) {
      if (!v.todos.length) {
        ...
        // The section itself carries the worst urgency present (danger beats
        // warning) as a tinted surface + top accent bar - not just another
        // white card with coloured icons.
        const worst = v.todos.some((t) => t.urgency === 'danger') ? 'danger' : 'warning';
        todoHost.innerHTML = `<div class="todo-surface todo-surface--${worst}">${v.todos.map((t) => `
          <div class="todo-row todo-row--${t.urgency}">
            <div class="todo-row__icon">${iconMarkup(t.icon)}</div>
            <div class="todo-row__title">${t.title}</div>
            <div class="todo-row__due">${t.due}</div>
          </div>`).join('')}</div>`;
      }
    }
    ...
    if (opsHost) {
      // Max 2 on the home screen - "Voir tout" covers the rest.
      opsHost.innerHTML = v.ops.slice(0, 2).map((o) => `
        <div class="list-row">
    ...
    const kmEl = document.querySelector('[data-carnet-health]');
    if (kmEl) kmEl.textContent = v.health;
    const expenseEl = document.querySelector('[data-carnet-expense]');
    if (expenseEl) expenseEl.textContent = v.expense;
    const maintEl = document.querySelector('[data-carnet-maint]');
    if (maintEl) maintEl.textContent = v.lastMaintWhen;
    const maintWhatEl = document.querySelector('[data-carnet-maint-what]');
    if (maintWhatEl) maintWhatEl.textContent = v.lastMaintWhat;
    const kmYearEl = document.querySelector('[data-carnet-kmyear]');
    if (kmYearEl) kmYearEl.textContent = v.kmYear;
